...
import { minorToUsdc, usdcToMinor } from '../ledger/casinoLedger.service';

export const CATALOG_STAKE_REVENUE = 'SYSTEM:USDC:CATALOG_STAKE_REVENUE';
export const CATALOG_PAYOUT_EXPENSE = 'SYSTEM:USDC:CATALOG_PAYOUT_EXPENSE';
...
const ensureCatalogAccounts = () => Promise.all([
  createSystemAccount(CATALOG_STAKE_REVENUE, 'REVENUE', 'USDC', 'CATALOG_STAKES'),
  createSystemAccount(CATALOG_PAYOUT_EXPENSE, 'EXPENSE', 'USDC', 'CATALOG_PAYOUTS')
]);
...
export const placeCatalogWager = async (ownerId: Types.ObjectId, input: { wagerId: string; sessionId: string; stakeSol: number }) => {
  ...
  const stakeMinor = usdcToMinor(input.stakeSol); if (stakeMinor <= 0n) throw new Error('Stake must be positive');
  ...
  if (wager) { if (wager.userId.toString() !== ownerId.toString() || wager.payloadHash !== payloadHash) throw new Error('Catalog wager idempotency payload mismatch'); if (wager.status !== 'FUNDS_RESERVED') return { wager, newBalance: minorToUsdc((await getUnifiedBalance(ownerId.toString(), 'USDC')).availableMinor) }; }
  else { await ensureCatalogAccounts(); await reserveFunds({ reservationId: `catalog:${input.wagerId}`, ownerId, currency: 'USDC', amountMinor: stakeMinor, referenceType: 'catalog_wager', referenceId: input.wagerId }); wager = await ExternalCasinoWager.create({ wagerId: input.wagerId, userId: ownerId, provider: session.provider, providerSessionId: session.providerSessionId, gameId: session.gameId, stakeMinor: decimal(stakeMinor), status: 'FUNDS_RESERVED', payloadHash }); }
  try {
    ...
    return { wager: settled, newBalance: minorToUsdc((await getUnifiedBalance(ownerId.toString(), 'USDC')).availableMinor) };
